...
final class Trie {
	 ...
	 * 
	 * @param loc the Node from where insertion should start.
	 * @param toAdd the String to add to the Trie data structure
	 * @param position from which insertion begins for the toAdd String
	 * 
	 */
	private static void insert(TrieNode loc, String toAdd, int letterPos) {
		if(toAdd != null && toAdd.length() > 0) { //not a null or empty string
			if(letterPos == toAdd.length()) { //last character, set word
				loc.setWord(toAdd);
			} else {
				TrieNode currNode = loc, found, newlyCreated;
				...
				if((found = currNode.findChildNode(charToFind)) == null) { //if the child node based on this current char doesn't exist, create one
					newlyCreated = new TrieNode(charToFind, currNode);
	 ...
	 * @return String suggested word, same as input if that is a valid word.
	 * 
	 */
	...
	/**
	 * Recursively looks for the exact match in the trie
	 * @param node
	 * @param toFind
	 * @param pos
	 * @return
	 */
	private static String simpleFind(TrieNode node, String toFind, int pos) {
	...
	/**
	 * 
	 * return string representation of trie for easier reading
	 */
	...
	/**
	 * Traverses the Trie children before node
	 * @param start
	 */
